...
class GomokuAI(Strategy):
    def __init__(self, N, depth=3):
        ...
        self.lastPlayed = 0
        self.emptyCells = N * N
        self.patternDict = create_pattern_dict() # dictionary containing all patterns with corresponding score
    ...
    def get_next_move(self, board, enemy_move: (int, int)) -> (int, int):
        ...
        if enemy_move is not None:
            self.boardValue = self.evaluate(enemy_move[0], enemy_move[1], self.boardValue, -1, self.nextBound)
            self.updateBound(enemy_move[0], enemy_move[1], self.nextBound)
            self.currentI, self.currentJ = enemy_move[0], enemy_move[1]
            # Make the move and update zobrist hash
            self.setState(enemy_move[0], enemy_move[1], -1)
            self.rollingHash ^= self.zobristTable[enemy_move[0]][enemy_move[1]][1]
            self.emptyCells -= 1
        self.alphaBetaPruning(3, self.boardValue, self.nextBound, -math.inf, math.inf, True)
        if self.isValid(self.currentI, self.currentJ):
            move_i, move_j = self.currentI, self.currentJ
            print(move_i, move_j)
            self.updateBound(move_i, move_j, self.nextBound)
        else:
            print('Error: i and j not valid. Given: ', self.currentI, self.currentJ)
            ...
            move_i = pos[0]
            move_j = pos[1]
            self.currentI, self.currentJ = move_i, move_j

            print(move_i, move_j)
        # Make the move and update zobrist hash
        self.setState(move_i, move_j, 1)
        self.rollingHash ^= self.zobristTable[move_i][move_j][0]
        self.emptyCells -= 1
        self.boardMap[move_i][move_j] = 1
        print(f"Updateing ai board {move_i}, {move_j}")
    ...
    def countDirection(self, i, j, xdir, ydir, state):
        ...
        for step in range(1, 5): 
            if xdir != 0 and (j + xdir*step < 0 or j + xdir*step >= self.N): # ensure move inside the board
                break
            if ydir != 0 and (i + ydir*step < 0 or i + ydir*step >= self.N):
                break
    ...
    def countPattern(self, i_0, j_0, pattern, score, bound, flag):
        ...
        for dir in directions:
            ...
            if dir[0] * dir[1] == 0:
                ...
                steps_back = min(5, self.N-1-j_0, i_0)
